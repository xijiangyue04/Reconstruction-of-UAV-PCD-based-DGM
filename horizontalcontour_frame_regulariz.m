%第三步：输入：line_number 为初始的横向或者纵向规则化几条轮廓线，默认是4条
%sort_R轮廓线密度，line_vector每个轮廓线的向量，boundary_pnts 边界点
%输出：horizon_regula 最终水平方向规则化轮廓线(元包形式)；vertical_regula 最终垂直方向规则化轮廓线(元包形式)，
%horizon_insertpnts 水平方向修补的内插点(矩阵形式)，vertical_insertpnts 垂直方向修补的内插点(矩阵形式)
%node_pnts 角点(矩阵形式)

function [horizon_regula,vertical_regula,horizon_insertpnts,vertical_insertpnts,node_pnts] = horizontalcontour_frame_regulariz(line_number,sort_R,boundary_pnts,line_vector,project_pnt,dist_thr) 
    resolution=0.005; %阈值2：描述的是规则化后点的相邻点间距为0.005米
    chose=1;
    number=10; %阈值3：描述的是规则化命令LFPR，至少要大于10个点才对其进行执行
%以下是依据轮廓线密度定位点和定位向量对横向主体轮廓线进行规则化
 multi_line_pnt=boundary_pnts(sort_R{1}(1:line_number,1),:);%对于line_vector每个向量，依据密度排序sort_R都要给定前line_number的点，有个点和向量就能确定线
...
for i=1:size(multi_line_pnt,1)
    ...
    outpnt=boundary_pnts(dist<0.1,:); %无人机是0.1
    segment{1}=outpnt;
    if  size(outpnt,1)>10
    Line_feature_segment = LFPR(segment,resolution,chose,number);
  ...
  for i=1:length(horizon_regula) %该数值是横向两个轮廓线
    ...
    for j=1:length(vertical_regula)%该数值是竖向两个轮廓线
      ...
      vertical_end=vertical_regula{j}(dist2==min_dist2,:);
        if min_dist1>0.1 & min_dist2>0.1 %无人机是0.1
...
%以下是判断以上方案得到内插轮廓边界线包络的边界点比率，如果包络了大部分边界点比如占比超过70%，则无需进行下面方案的轮廓内插
    ...
    vector=regular_pnts-project_pnt(i,:);%轮廓闭合点regular_pnts与内部投影点project_pnt之间的向量vector
    vec=vector./sqrt(sum(vector.^2,2));%vector向量单位化
    vec1=repmat(vec(1,:),m,1);%将第一个向量vec(1,:)作为初始向量
    degree=acos(dot(vec1',vec'));%所有向量与第一个初始向量夹角
...
%第二种方案
 %以下是对两个横向和竖向主体轮廓见空的部分进行横向和竖向直接连接内插的第二种方案的内插点
    if ratio<0.7 %阈值4：如果闭合轮廓线包络的边界点小于70%  无人机是0.7
    vertical_insertpnts=[];
    horizon_insertpnts=[];
    intersectpnts=[];
        for i=1:2
